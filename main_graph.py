...
def get_data(filename, sep='\t'):
    data = pd.read_csv(filename, sep=sep, header=None, names=['user_id', 'item_id', 'rating', 'timestamp'])
    return data
...
if __name__ == '__main__':
    ...
    Knn = 20 # change this for different Knn-Graphs
    
    print "==================================================================="
    print "================= You are testing for Knn=%d ======================" %Knn
    print "================= ......... Knn=%d ......... ======================"%Knn
    ...
    print 'TRAINING: get_training_Knn_Graph has finished! '
    
    # --- Predict ratings for test
    
    random_select = 1000 #based on the paper section 4.1.2
    N_list = [1, 5, 10, 15, 20] # number of recommended items

    tm = time.time()
    hits = -1*np.ones((test_size, len(N_list))) # 0: no hit, 1: hit in prediction
    ...
    recall = np.sum(hits[:ix,:], axis=0)/np.float(len(hits[:ix,0]))
    elapsed_time_avg = (time.time() - tm)/(ix+1)
    print '------  for N=%s, recall = %s' %(str(N_list), str(recall))
    print '------ Average Elapsed time per test case = %f sec ' %(elapsed_time_avg)
    print '======================== Knn=%d finished ===========================' %Knn
    print ''
    ...
    #%% ========================== GRAPH SECTION ===========================
    #%%
    # ====== These are results for the test above ======
    recom = [1,5,10,15,20]
    
    recall = [
                [0.044088176	,      0.026052104,	0.036072144,	0.024048096,	0.024048096],
                [0.108216433,	  0.116232465,	0.118236473,	0.086172345,	0.098196393],
                [0.134268537,	  0.180360721,	0.190380762,	0.154308617,	0.132264529],
                [0.138276553,	  0.214428858,	0.226452906,	0.190380762,	0.180360721],
                [0.138276553	,      0.226452906,	0.25250501,	      0.228456914,	0.218436874]
                ]
    plt.figure()
    plt.plot(recom, recall, '-o', linewidth=2)
    plt.xlabel('# of recommendations',fontsize=20)
    plt.ylabel('Recall', fontsize=20)
    plt.legend(['Knn=10','Knn=15','Knn=20','Knn=30','Knn=40'],loc=2)
    plt.show()
    
    
    #%%
    knn = np.array([10,15,20,30,40])
    time_avg = np.array([1.568528,1.890941,2.697144,4.092542,5.329186])
    plt.figure()
    plt.bar(left=knn-1.5, height=time_avg, width=3, color='b', edgecolor='c' )
    plt.xlabel('Parameter Knn',fontsize=20)
    plt.ylabel('Recommendation Time (sec)', fontsize=20)
